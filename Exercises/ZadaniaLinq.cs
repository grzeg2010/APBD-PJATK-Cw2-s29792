...
public sealed class ZadaniaLinq
{
    ...
    {
        var mails = DaneUczelni.Studenci.Select(s => s.Email);
        return mails;
    }
    ...
    {
        var imionaStudentow = DaneUczelni.Studenci
    ...
    {
        var analytics = DaneUczelni.Przedmioty
            .Where(p => p.Kategoria == "Analytics")
            .Select(p => $"{p.Nazwa}, {p.DataStartu}")
            .FirstOrDefault();

        //return analytics != null ? analytics : "Taki przedmiot nie istnieje";
        return analytics != null ? [analytics] : ["Nie ma takiego przedmiotu"];
    }
    ...
    {
        var activeCount = DaneUczelni.Zapisy.Count(e => e.CzyAktywny);
    ...
    {
        return DaneUczelni.Studenci.Select(e => e.Miasto).Distinct().Order();
    }
    ...
    public IEnumerable<string> Wyzwanie03_ProwadzacyISredniaOcenNaIchPrzedmiotach()
    {
        return DaneUczelni.Prowadzacy
            .GroupJoin(DaneUczelni.Przedmioty,
                pr => pr.Id,
                p => p.ProwadzacyId,
                (pr, p) => new { pr.Imie, pr.Nazwisko, p })
            .SelectMany(x => x.p.DefaultIfEmpty(),
                (x, p) => new { x.Imie, x.Nazwisko, PrzedmiotId = p != null ? p.Id : (int?)null })
            .GroupJoin(DaneUczelni.Zapisy,
                x => x.PrzedmiotId,
                z => z.PrzedmiotId,
                (x, z) => new { x.Imie, x.Nazwisko, z })
            .SelectMany(x => x.z.DefaultIfEmpty(),
                (x, z) => new
                {
                    x.Imie, x.Nazwisko, OcenaKoncowa = z != null ? z.OcenaKoncowa : null
                })
            .Where(x => x.OcenaKoncowa != null)
            .GroupBy(x => new { x.Imie, x.Nazwisko })
            .Select(g =>
                $"{g.Key.Imie}, {g.Key.Nazwisko}, {g.Average(x => x.OcenaKoncowa)}");
    }
    ...
    public IEnumerable<string> Wyzwanie04_MiastaILiczbaAktywnychZapisow()
    {
        return DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy,
                s => s.Id,
                z => z.StudentId,
                (stud, zap) => new { stud.Miasto, zap.CzyAktywny })
            .Where(x => x.CzyAktywny)
            .GroupBy(x => x.Miasto)
